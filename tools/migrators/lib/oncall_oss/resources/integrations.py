...
SYSTEM_MANAGED_INTEGRATION_TYPES = {"direct_paging"}


def is_system_managed_integration(integration: dict) -> bool:
    return integration.get("type") in SYSTEM_MANAGED_INTEGRATION_TYPES
...
def match_integration(integration: dict, oncall_integrations: List[dict]) -> None:
    ...
    For direct_paging integrations: match by name to an existing target DP.
    These cannot be created, only updated if already present in the target.
    """
    src_name = (integration.get("name") or "").lower().strip()
    oncall_integration = None

    for candidate in oncall_integrations:
        if src_name == (candidate.get("name") or "").lower().strip():
            oncall_integration = candidate
            break

    integration["oncall_integration"] = oncall_integration
    integration["migration_errors"] = []

    if is_system_managed_integration(integration) and oncall_integration is None:
        integration["migration_errors"] = [
            "Direct paging integrations are auto-created per team. "
            "Ensure the matching team exists in the target and rerun."
        ]


def _update_routes_on_existing_integration(
    integration: dict,
    routes: List[dict],
    chain_id_map: Dict[str, str],
) -> None:
    """Update routes on an existing target integration (e.g. matched direct_paging)."""
    target_id = integration["oncall_integration"]["id"]

    target_routes = OnCallAPIClient.list_all(
        f"routes/?integration_id={target_id}"
    )
...
def migrate_integration(
    ...
    For system-managed integrations that already exist in the target
    (e.g. direct_paging), only the routes are updated.
    """
    if is_system_managed_integration(integration):
        _update_routes_on_existing_integration(integration, routes, chain_id_map)
        return integration["oncall_integration"]
