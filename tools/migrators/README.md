# Grafana IRM migrator tools

These tools will help you to migrate from various on-call tools to Grafana IRM.
...
could be not migrated and some existing Grafana IRM resources could be deleted.
...
Integration report:
    ...
    ❌ DevOps - Email — cannot find appropriate Grafana IRM integration type
```
...
check if the resource already exists in Grafana IRM and will delete it before creating a new one.
...
- Delays between migrated notification/escalation rules could be slightly different from original.
  E.g. if you have a 4-minute delay between rules in PagerDuty, the resulting delay in Grafana IRM will be 5 minutes
...
Consider modifying [alert templates](https://grafana.com/docs/grafana-cloud/alerting-and-irm/irm/configure/escalation-routing/alert-templates/) of the created
...
with Grafana IRM, using your existing PagerDuty setup as a starting point for experimentation.

If this is relevant to you, you can migrate as such 👇

> [!IMPORTANT]
> As outlined several times in the documentation below, if you first import data into Grafana IRM without
> including users, make changes to that data within IRM, and then later re-import the data with users, Grafana IRM
...
| `EXPERIMENTAL_MIGRATE_EVENT_RULES`            | Migrate global event rulesets to Grafana IRM integrations.                                                                                                                                                                                                                                                                                                                                                      | Boolean                             | `false` |
...
The tool is capable of migrating user notification rules from PagerDuty to Grafana IRM.
Notification rules from the `"When a high-urgency incident is assigned to me..."` section in PagerDuty settings are
taken into account and will be migrated to both default and important notification rules in Grafana IRM
for each user. Note that delays between notification rules may be slightly different in Grafana IRM,
see [Limitations](#limitations) for more info.

By default (when `PRESERVE_EXISTING_USER_NOTIFICATION_RULES` is `true`), existing notification rules in Grafana IRM will
be preserved and PagerDuty rules won't be imported for users who already have notification rules configured in Grafana IRM.
...
The tool is capable of migrating on-call schedules from PagerDuty to Grafana IRM.
There are two ways to migrate on-call schedules:

- Migrate on-call shifts as if they were created in Grafana IRM web UI. Due to scheduling differences between
  PagerDuty and Grafana IRM, it's sometimes impossible to automatically migrate on-call shifts without manual changes
...
due to differences in scheduling systems in PD and Grafana IRM. To fix these errors, you need to manually change
on-call shifts in PD and re-run the migration.

#### Escalation policies

The tool is capable of migrating escalation policies from PagerDuty to Grafana IRM.
Every escalation policy will be migrated to a new Grafana IRM escalation chain with the same name.
...
Note that delays between escalation steps may be slightly different in Grafana IRM,
see [Limitations](#limitations) for more info.

#### Services (integrations)

The tool is capable of migrating services (integrations) from PagerDuty to Grafana IRM.
For every service in PD, the tool will migrate all integrations to Grafana IRM integrations.
...
- Elastic Alerts
- Firebase
- Amazon CloudWatch (maps to Amazon SNS integration in Grafana IRM)

#### Event rules (global event rulesets)

The tool is capable of migrating global event rulesets from PagerDuty to Grafana IRM integrations. This feature is
experimental and disabled by default. To enable it, set `EXPERIMENTAL_MIGRATE_EVENT_RULES` to `true`.

For every ruleset in PD, the tool will create a webhook integration in Grafana IRM. The tool will create
a route for every rule in ruleset, converting conditions in PD to Jinja2 routes in Grafana IRM. The tool will also
...
- When using `SCHEDULE_MIGRATION_MODE=ical`, at some point you would probably want to recreate schedules using
  Google Calendar or Terraform to be able to modify migrated on-call schedules in Grafana IRM
...
- Delays between migrated notification/escalation rules could be slightly different from original.
  E.g. if you have a 20-minute delay between rules in Splunk OnCall, the resulting delay in Grafana IRM will be 15 minutes
...
The tool is capable of migrating escalation policies from Splunk OnCall to Grafana IRM.
Every escalation policy will be migrated to a new Grafana IRM escalation chain with the same name.
...
- delays between escalation steps may be slightly different in Grafana IRM, see [Limitations](#limitations-1) for
  more info.
- the following Splunk OnCall escalation step types are not supported and will not be migrated:
  ...
  - "Execute webhook" (as Splunk OnCall webhooks are currently not migrated to Grafana IRM webhooks)

#### On-call schedules

The tool is capable of migrating on-call schedules from Splunk OnCall to Grafana IRM. Every Splunk On-Call Schedule
will be migrated to a new Grafana IRM schedule chain with the name as the Splunk team's name + `schedule`
(ex. `Infra Team schedule`).

Any existing Grafana IRM schedules with the same name will be deleted before migration.
...
The tool is capable of migrating paging policies from Splunk OnCall to Grafana IRM.
All user's **Primary** paging policy will be migrated to a new Grafana IRM user notification policy with the same name.
...
- The WhatsApp escalation type is not supported and will not be migrated to the Grafana IRM
...
The tool is capable of migrating user notification rules from Opsgenie to Grafana IRM.
Notification rules from Opsgenie will be migrated to both default and important notification rules in Grafana IRM
for each user. Note that delays between notification rules may be slightly different in Grafana IRM.

By default (when `PRESERVE_EXISTING_USER_NOTIFICATION_RULES` is `true`), existing notification rules in Grafana IRM will
be preserved and Opsgenie rules won't be imported for users who already have notification rules configured in Grafana IRM.
...
The tool is capable of migrating on-call schedules from Opsgenie to Grafana IRM.
...
The tool is capable of migrating escalation policies from Opsgenie to Grafana IRM.
Every escalation policy will be migrated to a new Grafana IRM escalation chain with name convention of
...
- Any existing escalation chains with the same name will be deleted, in Grafana IRM, before migration.
Note that delays between escalation steps may be slightly different in Grafana IRM
- Grafana IRM Escalation Policies which are migrated, are not attached to any Integration/Route, and must
be done manually
...
- Prometheus (maps to Alertmanager in Grafana IRM)
- PRTG
- Sentry
- Stackdriver
- UptimeRobot
...
Note that users are matched by email, so if there are users in the report with "no Grafana IRM user found with
